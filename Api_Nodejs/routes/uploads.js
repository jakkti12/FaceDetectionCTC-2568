...
router.post('/upload-single', authenticateToken, upload.single('picture'), async (req, res) => {
  try {
    if (!req.file) {
      return res.json({ ok: 0, error: "No file uploaded" });
    }
    ...
    res.json({
      ...
      file: picture // ส่งชื่อไฟล์กลับไป
    });
  } catch (error) {
...
router.post('/upload-profile-user', upload.single('picture'), async (req, res) => {
  try {
    const { user_id } = req.body;
    const picture = req.file.filename; // ชื่อไฟล์ที่อัปโหลด

    console.log(user_id);
    console.log(picture);

    // อัปเดตข้อมูลผู้ใช้ในฐานข้อมูล
    let ids =await req.db('users')
      .where({ id: user_id })
      .update({ profile: picture });

    res.status(200).json({ ok: 1, file: req.file, message: 'อัปโหลดไฟล์สำเร็จ', });
    console.log('out')
  } catch (error) {
